...
Функция ЗаполнитьСписокЗаказов(ДатаНачала, ДатаОкончания, СкладОтгрузки, ДополнительныеПараметры) Экспорт 
	...
	Если НЕ Результат.Пустой() Тогда 
		...
		Пока Выборка.Следующий() Цикл 
			...
			Если Настройки.Свойство("КоличествоМест") И Настройки.КоличествоМест = "Количество_Строк" Тогда
			...
			Если ЗначениеЗаполнено(Выборка.ВременноеОкноС) И ЗначениеЗаполнено(Выборка.ВременноеОкноПО) Тогда
				НоваяСтрока.ВременноеОкно = Формат(Выборка.ВременноеОкноС,"ДФ=ЧЧ:мм") + "-" + Формат(Выборка.ВременноеОкноПо,"ДФ=ЧЧ:мм");
			КонецЕсли;
...
&НаСервере
Функция ПолучитьКоличествоМест(Реализация)
	Кол = 0;
	Попытка
