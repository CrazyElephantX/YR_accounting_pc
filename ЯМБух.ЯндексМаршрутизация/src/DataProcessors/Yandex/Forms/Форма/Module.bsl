...
// Переменная соединения, что бы не устанавливать для каждого запроса новое, ускорение работы
&НаКлиенте
Перем Соединение;
...
Процедура ЗаполнитьСписок(Команда)	
	Если Конфигурация = "БИТУМЦ" Тогда
		...
		СписокВариантовОтбора.Добавить("Консультация","Консультация");
		СписокВариантовОтбора.Добавить("Анализы","Анализы");
	
		ПоказатьВопрос(Оповещение,"Какие заявки отобрать",СписокВариантовОтбора, 0);
	Иначе
		Доп = "";
...
Функция ПолучитьДополниеьльнуюИнформациюДляМониторинга(ДокументУИД)
	...
	Пока Выборка.Следующий() Цикл 
		...
		Иначе
			Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЯМ_ЗаявкаНаДоставку") Тогда
				КоличествоТоваров = 1;
			Иначе
