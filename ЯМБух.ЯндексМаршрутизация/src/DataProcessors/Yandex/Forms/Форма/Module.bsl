...
Процедура ПересчитатьМинутыВСекунды(Склады, ТочкиДоставки)
	...
	|	Справочник.ЯМ_Настройки КАК ЯМ_Настройки
	|ГДЕ
	|	ЯМ_Настройки.Имя = ""РасчетВМинутах""");
...
Функция ПолучитьДанныеМашинНаСервере(ТочкиДоставки, ТраспортныеСредства)
	...
	//ОбъектОбработка = РеквизитФормыВЗначение("Объект");
	//Возврат ОбъектОбработка.ПолучитьДанныеМашин(ТочкиДоставки);	
...
Функция ПолучитьДополниеьльнуюИнформациюДляМониторинга(ДокументУИД)
	...
	Пока Выборка.Следующий() Цикл 
		//ЗаполнитьЗначенияСвойств(ДопИнфо,Выборка);
		...
		Если Конфигурация = "БИТУМЦ" Тогда
			...
			Если Выборка.ВидУслуги = 0 Тогда
				//Комментарий = Комментарий + " Оплата: " + Выборка.Сумма;
	...
	Иначе
		...
		Если Результат.Свойство("result") Тогда
			...
			Для Каждого Маршрут Из Маршруты Цикл
				...
				//НоваяСтрока.id = ;
				//НоваяСтрока.imei = ;
				НоваяСтрокаМаршруты.number 			= Маршрут.vehicle_id + "-" + Маршрут.run_number + "-" + date; // Параметры.idПланирования +
				...
				Для Каждого ТочкаДоставки ИЗ ТочкиДоставки Цикл 
					...
					НоваяСтрокаТочкиДоставки.Транспорт				= НоваяСтрокаМаршруты.Транспорт;
					...
					Если (ТочкаДоставки.node.value.Свойство("type") 
						...
						Продолжить;
					КонецЕсли;
					НоваяСтрокаТочкиДоставки.ДопИнфо = ТочкаДоставки.node.value.id;
					ДопИнфо = ПолучитьДополниеьльнуюИнформациюДляМониторинга(ТочкаДоставки.node.value.id);		
					НоваяСтрокаТочкиДоставки.ТипТочки = НоваяСтрокаТочкиДоставки.ТипТочки + 
...
Функция СформироватьМаршрутныйЛистСКД(ТабДок,ПараметрыОтбора)
	...
	Настройки.ПараметрыДанных.Элементы[0].Значение = МассивСсылок;
    ...
    Настройки.ПараметрыВывода.Элементы.Найти("Заголовок").Значение              = ПараметрыОтбора.Машина;
